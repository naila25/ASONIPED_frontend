...
const AdminAnonymousTicketConversation: React.FC<AdminAnonymousTicketConversationProps> = ({
  ...
  return (
    <div className="bg-white flex min-w-0 flex-col border border-gray-100 rounded-lg overflow-hidden h-full min-h-0">
      ...
      {/* Messages Container */}
      <div className="flex-1 min-w-0 overflow-y-auto overflow-x-hidden bg-gray-50 px-4 py-6 min-h-0 messages-container">
        {loading ? (
          ...
          <>
            ...
            <div className="space-y-4">
              {messages.map((message) => (
                ...
                >
                  <div
                    className={`min-w-0 max-w-[75%] ${message.sender_type === 'admin' ? 'order-2' : 'order-1'}`}
                  >
                    {message.sender_type !== 'admin' && (
                      <div className="flex items-center gap-2 mb-1 ml-1 min-w-0">
